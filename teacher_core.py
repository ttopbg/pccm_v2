...
def _split_digits(grade, alpha, digits):
    ...
    if known_classes:
        splits = _enumerate_splits(grade, alpha, digits, known_clas ses)
        if len(splits) == 1:
            return splits[0]
        elif len(splits)  > 1:
...
def _compact(m):
    ...
    if d.endswith('0') and (not kno wn_classes or f "{g}{a}{d} " in known_classes):
        c = f "{g}{a}{d} "
        if c not in classes: classes.append(c)
        return ''
...
# Compact 3+ c hữ số: 10A123 → tách, 1A123 → tách (luôn là compact)
...
# Không có known_classes: k hông thể phân biệt → coi 2 digits là 1 lớp (an toàn hơn)
if known_classes:
    text = re.sub(r'(? <![\w,;])(0?[1-9]|1[0-2])([A-Za-z]+)(\d{2})(?![,;.\s\d])', _compact, text)

# ── Xử lý suffix groups: 11A4,5,11,12A6,7 → 11A4,11A5,11A11,12A6,12A7 ──
# Tokenize thành các nhóm (bas e, [suffixes]) rồi sinh lớp
...
def _parse_suffix_groups(seg):
    ...
    num tiếp theo là su ffix cho base hiện tại.
     " " "
...
for _, row in df.iterrows():
    ...
    for m in _ambig_pat.finditer(text):
        ...
        if token in known_classes:
            continue
        splits = _enumerate_splits(grade, alpha, d igits, known_classes)
        if len(splits)  <= 1:
...
for _, row in df.iterrows():
    ...
    for sr, ll in parsed:
        ...
        if code:
            continu e  # đã nhận diện được → bỏ qua
        key = sr.lower().strip()
        # Gợi ý fuzzy tốt nhất với cutoff thấp hơn để dễ suggest
        suggestion = None
        sn = _remove_acc ent(sr)
...
# Ví dụ: '11A4,5,11,12A6,7' → '11A4,11A5,1 1A11,12A6,12A7'
...
r'|'+_CLASS_PAT)                                                            # normal
tokens,results  = [],[]
tr = re.compile(r'(?P '+CRP+r')|(?P [+,\s]+)|(?P :)'
r'|(?P [A-Za-zÀ-ỹĐđ][A-Za-zÀ-ỹĐđ()]*)|(?P .)',re.UNICODE)
...
idx+=1
elif kind in ('s ep','colon','other'): idx+=1
else: idx+=1
...
df.columns = [str(c).strip() for c in df.column s]
...
for idx,row in df.iterrows():
    ...
    # Cấu hình môn chính khóa Tiểu học theo khối
    TH_CORE_SUBJECTS = {
        (1, 2, 3): ["TOAN", "TIENGVIET", "TUNHIENVAXAHOI", "DAODUC", "HDTN"],
        (4, 5):    ["TOAN", "TIENGVIET", "KHOAHOC", "LICHSUDIALI", "DAODUC", "HDTN"]
    }
    def _get_th_core_subs(grade):
        for grades, subs in TH_CORE_SUBJECTS.items():
            if grade in grades:
                return subs
        return []
    ...
    # 1. Nếu là cấp Tiểu học và có lớp chủ nhiệm -> Tự động gán môn chính khóa
    if cap_hoc == "TH" and cn_classes:
        for cls in cn_classes:
            grade = get_grade(cls)
            if grade:
                for sub in _get_th_core_subs(grade):
                    pair = (cls.strip(), sub)
    ...
    # 2. Gộp thêm các môn từ cột PCCM (nếu có)
    parsed = parse_pccm(praw, known_classes if known_classes else None, resolved_ambiguities or {})
    for sr, ll in parsed:
        ...
        if code:
            ...
            # Fallback nếu không map được môn
            for lop in ll:
                lop = lop.strip()
                if lop:
                    pair = (lop, sr.upper() if sr else "?")
                    if pair not in seen_pairs:
                        seen_pairs.add(pair)
                        mllist.append(pair)
...
for i,t in enumerate(teachers):
    rn=i+2
    wt.cell(row=rn,col umn=1,value=t[ "stt "])
